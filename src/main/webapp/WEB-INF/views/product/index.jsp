...
<div>
    <form action="/product/search${code}" method="get">
        <b>Mã sản phẩm </b><input name="code"/>
...
<table class="table">
    ...
    <c:forEach items="${list}" var="pd">
        <tr>
            ...
            <td>${pd.code}</td>
            <td>${pd.name}</td>
            <td>${pd.status?"Còn hàng":"Hết hàng"}</td>
            <td>
                <a href="/product/update/${pd.id}">Sửa</a>
                <a href="/product/delete/${pd.id}">Xóa</a>
                <a href="/product/detail/${pd.id}">Chi tiết</a>
            </td>
        </tr>
    </c:forEach>
    </tbody>
